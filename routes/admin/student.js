  ...
  async createStudent(req, res) {
    try {
      ...
      if (!_schoolId) {
        return res
          .status(400)
          .json({ error: true, message: "School ID is required" });
      }
      if (!dob) {
        return res
          .status(400)
      ...
      if (!rollNo) {
        return res
          .status(400)
          .json({ error: true, message: "Roll number is required" });
      }
      if (!_classId) {
        return res
          .status(400)
      ...
      const schoolName = await School.findOne({ _id: _schoolId })
        .select("name")
        .exec();
      ...
      const password =
        schoolName.name.slice(0, 3) + admissionYear + phone.slice(-3);
